...
# ADJUST THIS PATH IF YOUR SMALL LEXICON LIVES SOMEWHERE ELSE.
SMALL_LEXICON_CSV = PROJECT_ROOT / "annotation" / "lexicon" / "lexicon" / "Small_seed_lexicon.csv"

OUT_CSV = OUTPUT_DIR / "candidates_small.csv"

# Target learner L1s and CEFR levels
TARGET_L1S = ("Spanish", "French")
TARGET_CEFR = ("A1", "A2", "B1", "B2", "C1")

# Maximum number of essays per (L1, CEFR) cell       - This is something we can play around with if we want more examples 
N_PER_CELL = 10000


def load_small_lexicon(lexicon_path: Path):
    """Read the small lexicon and return:
    - surface_forms: set of surface word forms
    - lemmas: set of base lemmas
    - surface_to_lemma: mapping surface_form -> lemma
    """
    if not lexicon_path.exists():
        raise FileNotFoundError(f"Small lexicon not found: {lexicon_path}")
...
def main():
    ...
    surface_forms, lemmas, surface_to_lemma = load_small_lexicon(SMALL_LEXICON_CSV)
    print(f"Small lexicon: {len(lemmas)} lemmas, {len(surface_forms)} surface forms.")
