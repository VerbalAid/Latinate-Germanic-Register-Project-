...
def main() -> None:
    """
    Create a filtered CSV of "good" BIG-lexicon examples for human annotation.

    Input:
      - approach_a/output/preannotated_big.csv

    Output:
      - annotation/Big_to_Human_Annotate.csv

    The output file has:
      - Two instruction lines at the top (starting with '#')
      - One header row with column names and short hints in brackets
      - One row per selected example
    """
    project_root = Path(__file__).resolve().parents[1]
    pre_path = project_root / "approach_a" / "output" / "preannotated_big.csv"
    out_path = project_root / "annotation" / "Big_to_Human_Annotate.csv"
    ...
    # Some metadata columns might not exist in the BIG pipeline, or may exist
    ...
    if missing:
        raise KeyError(f"Missing expected columns in preannotated_big.csv: {missing}")
